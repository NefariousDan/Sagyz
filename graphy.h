           ...
           bool &redraw, unsigned int color, unsigned int axisTextColor)
{
  ...
  unsigned int acolor = GLSC_COL_TEAL;
  unsigned int pcolor = color;
  unsigned int tcolor = GLSC_COL_TEAL;
  unsigned int bcolor = COOL_GREY;
  ...
  if (redraw == true)
  {
    ...
    for (i = xlo; i <= xhi; i += xinc)
    {
      ...
      if (i == xlo) // 0 will not work if reading do not start from 0 so keep ylo the first point
      {
        ...
        tft.drawString(ylabel, (int)temp + 10, (int)(gy - h - 8), 1); //  using static value to adjust as per need
      }
      else
      {
      ...
      tft.drawFloat(i, 0, temp, gy - 10, 1); // disabled decimal in axis text
    }
           ...
           bool &update1, unsigned int color, unsigned int axisTextColor)
{
  ...
  unsigned int gcolor = GLSC_COL_TEAL;   // gcolor = graph grid color
  unsigned int acolor = GLSC_COL_TEAL;        // acolor = main axes and label color
  unsigned int pcolor = color;      // pcolor = color of your plotted data
  unsigned int tcolor = WHITE;      // tcolor = text color
  unsigned int bcolor = COOL_GREY;      // bcolor = background color
  ...
  if (update1)
  {
    ...
    for (i = ylo + 10; i <= yhi; i += yinc)
    {
      ...
      {
        tft.setTextColor(GLSC_COL_TEAL, bcolor);
        // tft.drawString(xlabel, (int)(gx + w) , (int)temp, 2);
        tft.drawString(xlabel, (int)(gx + w + 20), (310), 2); // Due to x axis padding moving away from axis
      }
      // draw the axis labels
      tft.setTextColor(tcolor, GLSC_COL_TEAL);
      // precision is default Arduino--this could really use some format control
      // tft.drawFloat(i, dp, gx - 4, temp, 1);
      tft.drawFloat(i, 0, gx - 11, temp, 1); // Changed to remove decimal places
    }
    ...
    for (i = xlo; i <= xhi; i += xinc)
    {
      ...
      { // 0 will not work if reading do not start from 0 so keep ylo the first point
        ...
        tft.setTextDatum(BC_DATUM);
        // tft.drawString(ylabel, (int)temp, (int)(gy - h - 8) , 2);
        tft.drawString(ylabel, (int)(temp - 5), (int)(gy - h - 8), 1); // Custom value for best fit
      }

      // draw the axis labels
      tft.setTextColor(axisTextColor, GLSC_COL_TEAL);
      tft.setTextDatum(TC_DATUM);
      // precision is default Arduino--this could really use some format control
      // tft.drawFloat(i, dp, temp, gy + 7, 1);
      tft.drawFloat(i, 0, temp, gy - 15, 1); // remove decimal by using 0
    }

    // now draw the graph labels
    tft.setTextColor(GLSC_COL_TEAL, bcolor);
    tft.drawString(title, (int)(gx + w / 2), (int)(gy - h) - 23, 2);
  }
...
void plotGraphP()
{
  ...
  update1 = true;
  tft.fillRect(30, 130, 280, 150, GLSC_COL_GRAY_L2); // need to wipe graph before each update else old traces will show, calculate or trial and error your coordinates
  tft.fillRect(30, 280, 280, 15, GLSC_COL_TEAL);  // x axis wiping is required as my x axis is dynamic
  tft.fillRect(6, 130, 19, 165, GLSC_COL_TEAL); // y axis is stable for me so no wiping: Hint - use visible color to wipe to see the area that is wiped (for testing)

  for (int u = 0; u < plotTemp.size(); u++)
  {
    yplot = plotTemp[u];
    xplot = plotTime[u];
    TraceDam(tft, xplot, yplot, 1, 35, 300, 275, 165, minX, (maxX + 1), xinc, minY, maxY, 10, "Graph", "Time(s)", ylabel, update1, WHITE, WHITE);
